...
Based on code base provided here: https://github.com/udacity/didi-competition

Process bags containing pointclouds and other sensors not handled in the Udacity provided code base (for example Radar)

Modified by Mark Strefford, Timelaps Robotics
...
#TODO - Modify this code to write out pointcloud top and panorama images
...
#ADDED
def write_pointcloud(outdir, msg):
    results = {}
    ...
    results['filename'] = pointcloud_filename
    return results
...
# ADDED
def pointcloud2dict(msg, write_results, pointcloud_dict):
    pointcloud_dict["timestamp"].append(msg.header.stamp.to_nsec())
    if write_results:
        pointcloud_dict["frame_id"].append(msg.header.frame_id)
        pointcloud_dict["filename"].append(write_results['filename'])
...
def main():
    ...
    for bs in bagsets:
        ...
        #TODO - Add in Radar, etc
        pointcloud_cols = ["timestamp", "frame_id", "filename"]
        pointcloud_dict = defaultdict(list)
        ...
        pointcloud_outdir = get_outdir(dataset_outdir, "pointcloud")  #ADDED
        # TODO - Add in other output dirs as required (Radar, etc.)
        ...
        def _process_msg(topic, msg, stats):
            ...
            # ADDED
            if topic in POINTCLOUD2_TOPICS:
                write_results = write_pointcloud(pointcloud_outdir, msg)
                pointcloud2dict(msg, write_results, pointcloud_dict)
                stats['msg_count'] += 1

            # TODO elif topic in RADAR_TOPICS: etc...
        ...
        for reader in readers:
            ...
            for result in reader.read_messages():
                ...
                if last_img_log != stats_acc['img_count'] and stats_acc['img_count'] % 1000 == 0:
                    print("%d pointclouds, processed..." % stats_acc['img_count'])
        ...
        pointcloud_df = pd.DataFrame(data=pointcloud_dict, columns=pointcloud_cols) #TODO Radar, etc
        ...
        pointcloud_df.to_csv(os.path.join(dataset_outdir, 'capture_vehicle_pointcloud.csv'), index=False)  #TODO Radar, etc.
        ...
        if len(pointcloud_dict['timestamp']):
        ...
        #
        #     #ADDED
        #     #print ('camera_df:{}'.format(camera_df))
        #     #print ('*********************')
        #
        #     camera_index_df = pd.DataFrame(index=camera_df.index)
        #
        #     #ADDED
        #     #print('camera_index_df:{}'.format(camera_index_df))
        #     #print('*************************')
        #
        ...
        #
        #     # ADDED
        #     #print ('cap_front_rtk_df:{}'.format(cap_front_rtk_df))
        #     #print('*************************')
        #
        ...
        #
        #     #ADDED
        #     #print('pointcloud_df:{}'.format(pointcloud_df))
        #     # if len(pointcloud_dict['timestamp']):
        #     #     pointcloud_interp = interpolate_to_camera(camera_index_df, pointcloud_df, filter_cols = pointcloud_cols)
        #     #     pointcloud_interp.to_csv(
        #     #         os.path.join(dataset_outdir, 'capture_vehicle_pointcloud_interp.csv'), header=True)
        #     #     pointcloud_interp_rec = pointcloud_interp.to_dict(orient='records')
        #
        ...
        #         plt.close(fig)
            ...
            print('Warning: No pointcloud times were found. '
